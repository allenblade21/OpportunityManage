...
- **统计报表**:周期切换(本月/本季/今年/全部)的六项指标(新增、赢单、输单、赢单率、平均成交周期、平均客单价),月度赢单趋势、在途阶段快照(含加权)、输单原因分布、来源分析、近 6 周跟进活跃度
- **标签筛选**:商机页按标签过滤看板与列表
...
**测试(方案见 [docs/test-plan.md](docs/test-plan.md)):** 86 个单元测试(流转联动、级联删除、Excel/JSON 往返、通知去重、IndexedDB 迁移、统计口径与周界/月界、金额/日期/日历边界等)+ 43 项 E2E 断言(拖拽、闭环、搜索、导出下载、导入回灌、坏文件防御、存储迁移、通知重载去重、统计页、标签筛选、移动端视口等),全部通过。

**规划中(见 [迭代路线](docs/design.md#9-迭代路线)):** 自定义阶段与标签管理、多用户后端(Node + Prisma + SQLite)。
